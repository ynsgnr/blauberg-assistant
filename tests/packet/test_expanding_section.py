...
@pytest.mark.parametrize(
    "new_value,expected", [(bytes([0xFF]), 0xFF), (bytes([0xFF, 0x11]), 0xFF11), (bytes([
        0x0, 0x0, 0x11]), 0x11), (bytes([
            0xDD, 0xFF, 0x11]), 0xDDFF11)]
)
...
@pytest.mark.parametrize(
    "new_value,expected", [(bytes([0xFF]), 0xFF), (bytes(
        [0xFF, 0x11]), 0xFF11), (bytes([0xFF, 0x11, 0x22]), 0xFF1122), ]
)
